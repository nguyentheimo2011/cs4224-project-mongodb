...
def process_delivery_transaction(db, w_id, carrier_id):
    ...
    for d_id in range(1, 11, 1):
        ...
        db.order.update_one({'o_w_num': w_id, 'o_d_num': d_id, 'o_num': next_o_id}, update_order, True)
    ...
    db.warehouse.update_one({'w_num': w_id}, update_warehouse, True)
